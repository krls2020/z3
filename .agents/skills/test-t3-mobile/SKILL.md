...
- Workspace: `<repo>/apps/mobile/ios/ZeropsCodeDev.xcworkspace`
- Scheme: `ZeropsCodeDev`
- Configuration: `Debug`
...
Use `snapshot_ui` and current element references from XcodeBuildMCP for taps and typing. Stream the same UDID through `ios-simulator-browser` so the user can watch in Zerops Code when the host supports it. Use the stream as a visual feed rather than a reason to switch to fragile browser coordinates.
...
3. Remove the disposable environment from Zerops Code Dev.
