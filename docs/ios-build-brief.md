...
## What is NOT done on mobile — expect T3 branding

The rebrand in this fork covers **web and desktop only**. If the goal is a Zerops-looking app on
the phone, that is unfinished work, not a bug:

- app names are still `T3 Code` / `T3 Code Dev` (`VARIANT_CONFIG` in `app.config.ts`)
- the wordmark is upstream's T3 glyph — `apps/mobile/src/components/T3Wordmark.tsx`, the same path
  data as the web sidebar had before it was replaced
...
- icons come from `assets/{dev,nightly,prod}` and are generated by macOS **Icon Composer**; the
  `.icns`/PNG export is explicitly not scriptable (`scripts/export-brand-icons.ts` prints manual
  steps). A human with the GUI is required for real icons.

If asked to rebrand mobile too, treat it as its own task and keep it additive, like the web side.
