...
it("presents showcase transports as remote endpoints", () => {
  ...
  assert.deepStrictEqual(
    ...
    ["https://moonbase.t3.sh/", "https://suspense-vps.hel1.t3.sh/", "http://100.82.16.5:3773/"],
  );
});
...
it("does not persist a cosmetic showcase URL when only the label is saved", () => {
  assert.equal(
    resolveShowcaseEnvironmentUpdateDisplayUrl({
      ...
      presentedDisplayUrl: "https://moonbase.t3.sh/",
      submittedDisplayUrl: "https://moonbase.t3.sh/",
    }),
    "http://127.0.0.1:3773/",
  );
  assert.equal(
