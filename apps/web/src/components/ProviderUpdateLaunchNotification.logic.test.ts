...
describe("provider update launch notification logic", () => {
  ...
  describe("isTerminalProviderUpdatePhase", () => {
    ...
    it("treats running/initial as non-terminal so they are not persisted", () => {
      // A "running"/"initial" snapshot never re-polls, so persisting it would
      // pin the update row's spinner forever once pending clears. Dropping
      // these lets the live provider state drive the row so it self-heals.
